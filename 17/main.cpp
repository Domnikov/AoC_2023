...
void AddNew(LL pos, LL score, Dir dir, LL dir_count, const VECI& path) {
    {LL newPos = pos; LL newScore = score; VECI newPath; if(USE_PATH){newPath.reserve(path.size()+2); newPath = path;} LL steps = (dir == Dir::Left ) ? 3 : 3-dir_count; for(LL i = 0;i< steps;++i){newPos = ToLeft (newPos); if(newPos == -1){break;} LL sc = GetScore(newPos); if(sc == -1){break;} newScore += sc; if(USE_PATH){newPath.push_back(newPos);}InsertIfLess(newPos, newScore, Dir::Left , dir_count+i, newPath);}}
    {LL newPos = pos; LL newScore = score; VECI newPath; if(USE_PATH){newPath.reserve(path.size()+2); newPath = path;} LL steps = (dir == Dir::Right) ? 3 : 3-dir_count; for(LL i = 0;i< steps;++i){newPos = ToRight(newPos); if(newPos == -1){break;} LL sc = GetScore(newPos); if(sc == -1){break;} newScore += sc; if(USE_PATH){newPath.push_back(newPos);}InsertIfLess(newPos, newScore, Dir::Right, dir_count+i, newPath);}}
    {LL newPos = pos; LL newScore = score; VECI newPath; if(USE_PATH){newPath.reserve(path.size()+2); newPath = path;} LL steps = (dir == Dir::Up   ) ? 3 : 3-dir_count; for(LL i = 0;i< steps;++i){newPos = ToUp   (newPos); if(newPos == -1){break;} LL sc = GetScore(newPos); if(sc == -1){break;} newScore += sc; if(USE_PATH){newPath.push_back(newPos);}InsertIfLess(newPos, newScore, Dir::Up   , dir_count+i, newPath);}}
    {LL newPos = pos; LL newScore = score; VECI newPath; if(USE_PATH){newPath.reserve(path.size()+2); newPath = path;} LL steps = (dir == Dir::Down ) ? 3 : 3-dir_count; for(LL i = 0;i< steps;++i){newPos = ToDown (newPos); if(newPos == -1){break;} LL sc = GetScore(newPos); if(sc == -1){break;} newScore += sc; if(USE_PATH){newPath.push_back(newPos);}InsertIfLess(newPos, newScore, Dir::Down , dir_count+i, newPath);}}
}
